...
const AP_Param::GroupInfo Tiltrotor::var_info[] = {
    ...
    // @Description: This is a bitmask of motors that are tiltable in a
    // tiltrotor (or tiltwing). The mask is in terms of the standard motor order
    // for the frame type.
    // @User: Standard
    // @Bitmask: 0:Motor 1, 1:Motor 2, 2:Motor 3, 3:Motor 4, 4:Motor 5, 5:Motor
    // 6, 6:Motor 7, 7:Motor 8, 8:Motor 9, 9:Motor 10, 10:Motor 11, 11:Motor 12
    AP_GROUPINFO("MASK", 2, Tiltrotor, tilt_mask, 0),

    // @Param: RATE_UP
    // @DisplayName: Tiltrotor upwards tilt rate
    // @Description: This is the maximum speed at which the motor angle will
    // change for a tiltrotor when moving from forward flight to hover
    // @Units: deg/s
    // @Increment: 1
    // @Range: 10 300
    // @User: Standard
    ...
    // @Description: This is the maximum angle of the tiltable motors at which
    // multicopter control will be enabled. Beyond this angle the plane will fly
    // solely as a fixed wing aircraft and the motors will tilt to their maximum
    // angle at the TILT_RATE
    // @Units: deg
    // @Increment: 1
    // @Range: 20 80
    // @User: Standard
    ...
    // @Description: This is the type of tiltrotor when TILT_MASK is non-zero. A
    // continuous tiltrotor can tilt the rotors to any angle on demand. A binary
    // tiltrotor assumes a retract style servo where the servo is either fully
    // forward or fully up. In both cases the servo can't move faster than
    // Q_TILT_RATE. A vectored yaw tiltrotor will use the tilt of the motors to
    // control yaw in hover, Bicopter tiltrotor must use the tailsitter frame
    // class (10)
    ...
    // @Description: This is the maximum speed at which the motor angle will
    // change for a tiltrotor when moving from hover to forward flight. When
    // this is zero the Q_TILT_RATE_UP value is used.
    // @Units: deg/s
    // @Increment: 1
    // @Range: 10 300
    // @User: Standard
    ...
    // @Description: This is the angle of the tilt servos when in VTOL mode and
    // at minimum output (fully back). This needs to be set in addition to
    // Q_TILT_TYPE=2, to enable vectored control for yaw in tilt quadplanes.
    // This is also used to limit the forward travel of bicopter
    // tilts(Q_TILT_TYPE=3) when in VTOL modes.
    // @Range: 0 30
    AP_GROUPINFO("YAW_ANGLE", 7, Tiltrotor, tilt_yaw_angle, 0),

    // @Param: FIX_ANGLE
    // @DisplayName: Fixed wing tiltrotor angle
    // @Description: This is the angle the motors tilt down when at maximum
    // output for forward flight. Set this to a non-zero value to enable
    // vectoring for roll/pitch in forward flight on tilt-vectored aircraft
    // @Units: deg
    // @Range: 0 30
    // @User: Standard
    ...
    // @Description: This is the gain for use of tilting motors in fixed wing
    // flight for tilt vectored quadplanes
    // @Range: 0 1
    // @User: Standard
    ...
    // @Description: For use on tilt wings, the wing will tilt up to this angle
    // for flap, transition will be complete when the wing reaches this angle
    // from the forward fight position, 0 disables
    // @Units: deg
    // @Increment: 1
    // @Range: 0 15
    // @User: Standard
...
Tiltrotor::Tiltrotor(QuadPlane &_quadplane, AP_MotorsMulticopter *&_motors)
    : quadplane(_quadplane), motors(_motors) {
...
void Tiltrotor::setup() {
  ...
  if (!enable.configured() &&
      ((tilt_mask != 0) || (type == TILT_TYPE_BICOPTER))) {
  ...
  // true if a fixed forward motor is configured, either throttle, throttle left
  // or throttle right. bicopter tiltrotors use throttle left and right as
  // tilting motors, so they don't count in that case.
  _have_fw_motor =
  ...
  if (tilt_mask != 0) {
    ...
    motors->set_thrust_compensation_callback(FUNCTOR_BIND_MEMBER(
        &Tiltrotor::tilt_compensate, void, float *, uint8_t));
...
void Tiltrotor::slew(float newtilt) {
  float max_change =
      tilt_max_change(newtilt<current_tilt, newtilt> get_fully_forward_tilt());
  current_tilt = constrain_float(newtilt, current_tilt - max_change,
                                 current_tilt + max_change);
...
void Tiltrotor::continuous_update(void) {
  ...
  if (!quadplane.in_vtol_mode() &&
      (!plane.arming.is_armed_and_safety_off() || !quadplane.assisted_flight)) {
    // we are in pure fixed wing mode. Move the tiltable motors all the way
    // forward and run them as a forward motor

    // option set then if disarmed move to VTOL position to prevent ground
    // strikes, allow tilt forward in manual mode for testing
    const bool disarmed_tilt_up =
        !plane.arming.is_armed_and_safety_off() &&
        (plane.control_mode != &plane.mode_manual) &&
        quadplane.option_is_set(QuadPlane::Option::DISARMED_TILT_UP);
    slew(disarmed_tilt_up ? 0.0 : get_forward_flight_tilt());

    max_change = tilt_max_change(false);

    float new_throttle = constrain_float(
        SRV_Channels::get_output_scaled(SRV_Channel::k_throttle) * 0.01, 0, 1);
    if (current_tilt < get_fully_forward_tilt()) {
      current_throttle =
          constrain_float(new_throttle, current_throttle - max_change,
  ...
  current_throttle =
      constrain_float(motors_throttle, current_throttle - max_change,
                      current_throttle + max_change);
  ...
  /*
    ...
    1) With use of a forward throttle controlled by Q_FWD_THR_GAIN in
       VTOL modes except Q_AUTOTUNE determined by Q_FWD_THR_USE. We set the
    angle based on a calculated forward throttle.
  ...
  */
  ...
  if (!quadplane.assisted_flight &&
    ...
    // We are using the rotor tilt functionality controlled by Q_FWD_THR_GAIN
    // which can operate in all VTOL modes except Q_AUTOTUNE. Forward rotor tilt
    // is used to produce forward thrust equivalent to what would have been
    // produced by a forward thrust motor set to
    // quadplane.forward_throttle_pct()
    const float fwd_g_demand = 0.01 * quadplane.forward_throttle_pct();
    const float fwd_tilt_deg =
        MIN(degrees(atanf(fwd_g_demand)), (float)max_angle_deg);
    ...
    if (quadplane.rc_fwd_thr_ch == nullptr) {
      ...
      slew(
          MIN(settilt * max_angle_deg * (1 / 90.0), get_forward_flight_tilt()));
    }
    return;
  }
  ...
  if (quadplane.assisted_flight &&
    ...
    float settilt = constrain_float(
        (SRV_Channels::get_output_scaled(SRV_Channel::k_throttle) -
         MAX(plane.aparm.throttle_min.get(), 0)) *
            0.02,
        0, 1);
...
void Tiltrotor::update(void) {
  ...
  // HERE: continuous runs for vectored yaw as well? i think disabling the yaw
  // just doesn't let it yaw
...
void Tiltrotor::write_log() {
  ...
  struct log_tiltrotor pkt {
    LOG_PACKET_HEADER_INIT(LOG_TILT_MSG),
        time_us : AP_HAL::micros64(), current_tilt : current_tilt * 90.0,
  };
  ...
  if (type != TILT_TYPE_VECTORED_YAW) {
    ...
    pkt.front_left_tilt =
        (SRV_Channels::get_output_scaled(SRV_Channel::k_tiltMotorLeft) *
         scale) -
        tilt_yaw_angle;
    pkt.front_right_tilt =
        (SRV_Channels::get_output_scaled(SRV_Channel::k_tiltMotorRight) *
         scale) -
        tilt_yaw_angle;
  }
...
/*
  ...
  1) when we are transitioning to fwd flight we scale the tilted rotors by
  1/cos(angle). This pushes us towards more flight speed

  2) when we are transitioning to hover we scale the non-tilted rotors by
  cos(angle). This pushes us towards lower fwd thrust
 ...
 */
void Tiltrotor::tilt_compensate_angle(float *thrust, uint8_t num_motors,
                                      float non_tilted_mul, float tilted_mul) {
  float tilt_total = 0;
  ...
  for (uint8_t i = 0; i < num_motors; i++) {
    if (is_motor_tilting(i)) {
      ...
      thrust[i] =
          current_tilt * avg_tilt_thrust + thrust[i] * (1 - current_tilt);
      // add in differential thrust for yaw control, scaled by tilt angle
      const float diff_thrust = motors->get_roll_factor(i) *
...
void Tiltrotor::vectoring(void) {
  ...
  if (!plane.arming.is_armed_and_safety_off() &&
      plane.quadplane.option_is_set(QuadPlane::Option::DISARMED_TILT)) {
    // this test is subject to wrapping at ~49 days, but the consequences are
    // insignificant
    if ((now - hal.util->get_last_armed_change()) > TILT_DELAY_MS) {
      if (quadplane.in_vtol_mode()) {
        ...
        const float right =
            gain *
            SRV_Channels::get_output_scaled(SRV_Channel::k_elevon_right) *
            (1 / 4500.0);
        const float left =
            gain * SRV_Channels::get_output_scaled(SRV_Channel::k_elevon_left) *
            (1 / 4500.0);
        const float mid =
            gain * SRV_Channels::get_output_scaled(SRV_Channel::k_elevator) *
            (1 / 4500.0);
        // front tilt is effective canards, so need to swap and use negative.
        // Rear motors are treated live elevons.
        SRV_Channels::set_output_scaled(
            SRV_Channel::k_tiltMotorLeft,
            1000 * constrain_float(base_output - right, 0, 1));
        SRV_Channels::set_output_scaled(
            SRV_Channel::k_tiltMotorRight,
            1000 * constrain_float(base_output - left, 0, 1));
        SRV_Channels::set_output_scaled(
            SRV_Channel::k_tiltMotorRearLeft,
            1000 * constrain_float(base_output + left, 0, 1));
        SRV_Channels::set_output_scaled(
        ...
        SRV_Channels::set_output_scaled(
            ...
            1000 * constrain_float(base_output + mid, 0, 1));
      }
    }
    return;
  }
  ...
  if (no_yaw) {
    // fixed wing  We need to apply inverse scaling with throttle, and remove
    // the surface speed scaling as we don't want tilt impacted by airspeed
    const float scaler = plane.control_mode == &plane.mode_manual
                             ? 1
                             : (quadplane.FW_vector_throttle_scaling() /
                                plane.get_speed_scaler());
    const float gain = fixed_gain * fixed_tilt_limit * scaler;
    const float right =
        gain * SRV_Channels::get_output_scaled(SRV_Channel::k_elevon_right) *
        (1 / 4500.0);
    const float left =
        gain * SRV_Channels::get_output_scaled(SRV_Channel::k_elevon_left) *
        (1 / 4500.0);
    const float mid = gain *
    ...
    if (is_positive(throttle)) {
      throttle_scaler = constrain_float(motors->get_throttle_hover() / throttle,
                                        scale_min, scale_max);
    }
    ...
    float tilt_scale = throttle_scaler * yaw_out * cos_tilt +
                       avg_roll_factor * roll_out * sin_tilt;
    ...
    SRV_Channels::set_output_scaled(SRV_Channel::k_tiltMotorRear,
                                    1000.0 *
                                        constrain_float(base_output, 0.0, 1.0));
...
void Tiltrotor::bicopter_output(void) {
  ...
  tilt_left = constrain_float(-(current_tilt * SERVO_MAX) + tilt_left,
                              -SERVO_MAX, SERVO_MAX);
  tilt_right = constrain_float(-(current_tilt * SERVO_MAX) + tilt_right,
                               -SERVO_MAX, SERVO_MAX);
...
void Tiltrotor::update_yaw_target(void) {
  ...
  if (have_airspeed && labs(plane.nav_roll_cd) > 1000) {
    ...
    const float yaw_rate_cds = fixedwing_turn_rate(plane.nav_roll_cd * 0.01,
                                                   MAX(aspeed, airspeed_min)) *
                               100;
...
bool Tiltrotor_Transition::use_multirotor_control_in_fwd_transition() const {
  if (tiltrotor.disable_mc_control() || !tiltrotor.is_vectored()) {
    return false;
  }
...
bool Tiltrotor_Transition::show_vtol_view() const {
  ...
  if (!show_vtol && !tiltrotor.disable_mc_control() && tiltrotor.is_vectored() &&
...
// Return true if forward throttle should be allowed for position control, see
// Q_FWD_THR_USE
bool Tiltrotor_Transition::allow_vfwd() const {
  // Don't allow forward throttle (tilt) if a tilting motor has failed which
  // would result in a yaw imbalance
  ...
  // Failed tilting motor is on the center line, so will not cause a yaw
  // imbalance
...
// return true if forward transition should stop using multicopter control
bool Tiltrotor::disable_mc_control() const {
  return enabled() && !is_vectored() && tilt_over_max_angle();
}
...
bool Tiltrotor::get_forward_throttle(float &throttle) const {
  ...
  const float throttle_range =
      motors->thr_lin.get_spin_max() - motors->thr_lin.get_spin_min();
